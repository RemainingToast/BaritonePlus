...
import baritone.plus.main.tasks.anarchy.duping.TameDonkeyTask;
import baritone.plus.main.tasks.container.StoreInContainerTask;
import baritone.plus.main.tasks.movement.FastTravelTask;
...
public class EscapeSpawnTask extends Task {
    ...
    private static BlockPos generatePosition(BaritonePlus mod, int originalRange, int newPositionRange) {
    ...
    private boolean isWithinRange(BaritonePlus mod, int range) {
        var player = mod.getPlayer();
    ...
    private boolean isOutsideRange(BaritonePlus mod, int range) {
        var player = mod.getPlayer();
        // Check if the absolute value of player's block positions are within the range
        return Math.abs(player.getBlockX()) >= range &&
                Math.abs(player.getBlockY()) >= range &&
    ...
    public BlockPos findClosestNetherHighway(BaritonePlus mod) {
        var player = mod.getPlayer();
