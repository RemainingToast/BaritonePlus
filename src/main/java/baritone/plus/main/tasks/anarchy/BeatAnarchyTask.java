package baritone.plus.main.tasks.anarchy;
...
import baritone.plus.main.tasks.misc.EquipArmorTask;
import baritone.plus.main.tasks.misc.LootDesertTempleTask;
import baritone.plus.main.tasks.misc.PlaceBedAndSetSpawnTask;
import baritone.plus.main.tasks.misc.SleepThroughNightTask;
import baritone.plus.main.tasks.movement.*;
import baritone.plus.main.tasks.resources.*;
import baritone.plus.main.tasks.speedrun.BeatMinecraftConfig;
...
// Needs Alot of Work
// Should
// (Collecting Useful Items on the Way like BeatMinecraftTask)
// - Escape Inner Spawn Region (<10k),
// - Get Food,
// - Starter Gear,
// - Then Goto Desired Base Location
@SuppressWarnings("ALL")
public class BeatAnarchyTask extends Task {
    ...
    private static FastTravelTask _locateStrongholdTask;
    ...
    public BeatAnarchyTask() {
        _locateStrongholdTask = new FastTravelTask(new BlockPos(420000, 69, 420000), true);
    ...
    private static boolean shouldForce(BaritonePlus mod, Task task) {
    ...
    private boolean hasItemOrDroppedInEnd(BaritonePlus mod, Item item) {
        return mod.getItemStorage().hasItem(item) || droppedInEnd(item);
    }

    private List<Item> lootableItems(BaritonePlus mod) {
    ...
    protected boolean isEqual(Task other) {
        return other instanceof BeatAnarchyTask;
    }

    @Override
    protected String toDebugString() {
        return "Beating Anarchy.";
    }

    private boolean endPortalFound(BaritonePlus mod, BlockPos endPortalCenter) {
        return false;
...
//        return getFrameBlocks(endPortalCenter).stream().anyMatch(frame -> mod.getBlockTracker().blockIsValid(frame, Blocks.END_PORTAL_FRAME));
    ...
    private boolean endPortalOpened(BaritonePlus mod, BlockPos endPortalCenter) {
        return _endPortalOpened && endPortalCenter != null && mod.getBlockTracker().blockIsValid(endPortalCenter, Blocks.END_PORTAL);
    }

    private boolean spawnSetNearPortal(BaritonePlus mod, BlockPos endPortalCenter) {
...
//        }
        ...
        switch (WorldHelper.getCurrentDimension()) {
            case OVERWORLD -> {
                ...
                if (shouldForce(mod, _foodTask)) {
                    setDebugState("Getting food for journey.");
                    return _foodTask;
                } else {
                    _foodTask = null;
                }
                ...
                if (shouldForce(mod, _gearTask) && !StorageHelper.isArmorEquipped(mod, COLLECT_EYE_ARMOR)) {
                    setDebugState("Getting diamond gear for the journey.");
                    return _gearTask;
                } else {
                    _gearTask = null;
                }

                boolean eyeGearSatisfied1 = StorageHelper.itemTargetsMet(mod, COLLECT_EYE_GEAR_MIN) && StorageHelper.isArmorEquippedAll(mod, COLLECT_EYE_ARMOR);
                boolean ironGearSatisfied1 = StorageHelper.itemTargetsMet(mod, COLLECT_IRON_GEAR_MIN) && StorageHelper.isArmorEquippedAll(mod, COLLECT_IRON_ARMOR);
                ...
                if (!mod.getItemStorage().hasItem(Items.PORKCHOP) &&
                        ...
                        !StorageHelper.itemTargetsMet(mod, IRON_GEAR_MIN) && !ironGearSatisfied1 && !eyeGearSatisfied1) {
                    if (mod.getItemStorage().getItemCount(ItemHelper.LOG) < 12 && !StorageHelper.itemTargetsMet(mod, COLLECT_STONE_GEAR) &&
                            !StorageHelper.itemTargetsMet(mod, IRON_GEAR_MIN) && !eyeGearSatisfied1 &&
                            !ironGearSatisfied1) {
                    ...
                    if (mod.getEntityTracker().entityFound(PigEntity.class) && (StorageHelper.itemTargetsMet(mod,
                            ...
                            eyeGearSatisfied1 || ironGearSatisfied1)) {
                ...
                // Then starter gear
                if (!StorageHelper.itemTargetsMet(mod, IRON_GEAR_MIN) && !eyeGearSatisfied1 &&
                        !ironGearSatisfied1) {
                ...
                // Then get iron
                if (_config.ironGearBeforeDiamondGear && !ironGearSatisfied1 && !eyeGearSatisfied1 &&
        ...
        switch (WorldHelper.getCurrentDimension()) {
            case OVERWORLD -> {
                ...
                if (!mod.getItemStorage().hasItem(Items.WATER_BUCKET)) {
                    setDebugState("Getting MLG water bucket.");
    ...
    private Task setSpawnNearPortalTask(BaritonePlus mod) {
    ...
    private Task getBlazeRodsTask(BaritonePlus mod, int count) {
    ...
    private Task getEnderPearlTask(BaritonePlus mod, int count) {
    ...
    private int getTargetBeds(BaritonePlus mod) {
    ...
    private boolean needsBeds(BaritonePlus mod) {
        return false;
//        int inEnd = 0;
...
//        return (mod.getItemStorage().getItemCount(ItemHelper.BED) + inEnd) < getTargetBeds(mod);
    ...
    private Task getBedTask(BaritonePlus mod) {
    ...
    private boolean anyBedsFound(BaritonePlus mod) {
    ...
    private BlockPos doSimpleSearchForEndPortal(BaritonePlus mod) {
    ...
    private int getFilledPortalFrames(BaritonePlus mod, BlockPos endPortalCenter) {
