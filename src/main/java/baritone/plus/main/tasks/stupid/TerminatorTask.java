...
import baritone.plus.main.tasks.movement.RunAwayFromEntitiesTask;
import baritone.plus.main.tasks.movement.SearchChunksExploreTask;
...
public class TerminatorTask extends Task {
    ...
    private boolean isReadyToPunk(BaritonePlus mod) {
    ...
    private boolean shouldPunk(BaritonePlus mod, PlayerEntity player) {
        if (player == null || player.isDead()) return false;
        if (player.isCreative() || player.isSpectator()) return false;
        return !mod.getButler().isUserAuthorized(player.getName().getString()) && _canTerminate.test(player);
    }

    private void tryDoFunnyMessageTo(BaritonePlus mod, PlayerEntity player) {
