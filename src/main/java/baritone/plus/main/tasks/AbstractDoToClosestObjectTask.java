...
public abstract class AbstractDoToClosestObjectTask<T> extends Task {
    ...
    protected abstract Vec3d getPos(BaritonePlus mod, T obj);

    protected abstract Optional<T> getClosestTo(BaritonePlus mod, Vec3d pos);

    protected abstract Vec3d getOriginPos(BaritonePlus mod);

    protected abstract Task getGoalTask(T obj);

    protected abstract boolean isValid(BaritonePlus mod, T obj);
    ...
    private double getCurrentCalculatedHeuristic(BaritonePlus mod) {
    ...
    private boolean isMovingToClosestPos(BaritonePlus mod) {
