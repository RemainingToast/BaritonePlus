...
abstract class ChunkSearchTask extends Task {
    ...
    // Virtual
    protected ChunkPos getBestChunk(BaritonePlus mod, List<ChunkPos> chunks) {
    ...
    private void searchChunkOrQueueSearch(BaritonePlus mod, ChunkPos pos) {
     ...
     */
    private boolean trySearchChunk(BaritonePlus mod, ChunkPos pos) {
        // Do NOT search later.
        if (_searchedAlready.contains(pos)) {
            return true;
        }
    ...
    protected abstract boolean isChunkPartOfSearchSpace(BaritonePlus mod, ChunkPos pos);
