...
import baritone.plus.main.tasks.construction.compound.ConstructNetherPortalBucketTask;
...
public class DefaultGoToDimensionTask extends Task {
    ...
    private Task goToOverworldFromNetherTask(BaritonePlus mod) {
    ...
    private Task goToOverworldFromEndTask(BaritonePlus mod) {
        setDebugState("TODO: Go to center portal (at 0,0). If it doesn't exist, kill ender dragon lol");
        return null;
    }

    private Task goToNetherFromOverworldTask(BaritonePlus mod) {
        if (netherPortalIsClose(mod)) {
            setDebugState("Going to nether portal");
            return new EnterNetherPortalTask(Dimension.NETHER);
        }
    ...
    private boolean netherPortalIsClose(BaritonePlus mod) {
