...
import baritone.plus.api.util.serialization.IFailableConfigFile;
...
public class PlusSettings implements IFailableConfigFile {
    ...
    public static void load(Consumer<PlusSettings> onReload) {
        ConfigHelper.loadConfig(SETTINGS_PATH, PlusSettings::new, PlusSettings.class, onReload);
    }
    ...
    public Item[] getThrowawayItems(BaritonePlus mod, boolean includeProtected) {
        return throwawayItems.stream().filter(item -> includeProtected || !mod.getBehaviour().isProtected(item)).toArray(Item[]::new);
    }

    public Item[] getThrowawayItems(BaritonePlus mod) {
