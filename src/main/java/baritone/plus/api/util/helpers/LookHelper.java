...
public interface LookHelper {
    ...
    static Optional<Rotation> getReach(BlockPos target, EnumFacing side) {
    ...
    static boolean cleanLineOfSight(Vec3d end, double maxRange) {
        return cleanLineOfSight(Minecraft.getMinecraft().player, end, maxRange);
    }
    ...
    static boolean cleanLineOfSight(Entity entity, BlockPos block, double maxRange) {
        ...
        return switch (hit.getType()) {
            case MISS -> true;
            case BLOCK -> hit.getPosition().equals(block);
            case ENTITY -> false;
        };
    }
    ...
    static Rotation getLookRotation() {
        ...
        return getLookRotation(Minecraft.getMinecraft().player);
    }
    ...
    static Vec3d getCameraPos(BaritonePlus mod) {
        IPlayerContext ctx = BaritoneAPI.getProvider().getPrimaryBaritone().getPlayerContext();
    ...
    static boolean tryAvoidingInteractable(BaritonePlus mod) {
    ...
    private static boolean isCollidingInteractable(BaritonePlus mod) {
        ...
        HitResult result = Minecraft.getMinecraft().crosshairTarget;
    ...
    static void randomOrientation(BaritonePlus mod) {
        Rotation r = new Rotation((float) Math.random() * 360f, -90 + (float) Math.random() * 180f);
        lookAt(mod, r);
    }

    static boolean isLookingAt(BaritonePlus mod, Rotation rotation) {
        return rotation.isReallyCloseTo(getLookRotation());
    }

    static boolean isLookingAt(BaritonePlus mod, BlockPos blockPos) {
        return mod.getClientBaritone().getPlayerContext().isLookingAt(blockPos);
    }

    static void lookAt(BaritonePlus mod, Rotation rotation) {
    ...
    static void lookAt(BaritonePlus mod, Vec3d toLook) {
        Rotation targetRotation = getLookRotation(mod, toLook);
        lookAt(mod, targetRotation);
    }

    static void lookAt(BaritonePlus mod, BlockPos toLook, EnumFacing side) {
    ...
    static void lookAt(BaritonePlus mod, BlockPos toLook) {
        lookAt(mod, toLook, null);
    }

    static Rotation getLookRotation(BaritonePlus mod, Vec3d toLook) {
    ...
    static Rotation getLookRotation(BaritonePlus mod, BlockPos toLook) {
